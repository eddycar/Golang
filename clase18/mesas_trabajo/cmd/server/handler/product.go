...
func (h *productHandler)ProductsPriceSum() gin.HandlerFunc{
	return func(ctx *gin.Context) {
		idList := ctx.Query("list")
	    idListInt := make([]int, len(idList))
		for _, v := range idList{
			id, err := strconv.Atoi(string(v))
			if err != nil {
				panic(errors.New("No se pudo convertir"))
			}
			idListInt = append(idListInt, id)
		}
	
		// products := h.s.ProductsSum(idListInt)
